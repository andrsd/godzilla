...
TEST_F(InitialConditionTest, test)
{
    ...
    EXPECT_TRUE(prob->has_initial_condition("obj"));
    auto ic2 = prob->get_initial_condition("obj");
    EXPECT_TRUE(ic2.has_value());
    EXPECT_EQ(ic2.value(), ic);

    EXPECT_FALSE(prob->has_initial_condition("non-existent-ic"));
    auto ic3 = prob->get_initial_condition("non-existent-ic");
    ...
    EXPECT_TRUE(prob->has_initial_condition("a_ic"));
    auto aic2 = prob->get_initial_condition("a_ic");
...
TEST_F(InitialConditionTest, duplicate_ic_name)
{
    ...
    EXPECT_DEATH(prob->add_initial_condition<TestInitialCondition>(params),
...
TEST_F(InitialCondition2FieldTest, non_existing_field)
{
    ...
    EXPECT_DEATH(prob->create(), "Field 'asdf' does not exist. Typo?");
}
