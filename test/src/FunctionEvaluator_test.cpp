...
TEST(FunctionEvaluatorTest, multi_evaluate)
{
    ...
    Real x[] = { 3, 4 };
    Real val[2] = { 0., 0. };
    EXPECT_TRUE(e.evaluate(2_D, 3., x, 2, val));
...
TEST(FunctionEvaluatorTest, error)
{
    ...
    e.create("a");
    Real x[] = { 1 };
    EXPECT_THROW_MSG(e.evaluate(1_D, 0., x),
...
TEST(FunctionEvaluatorTest, evaluate_error)
{
    ...
    Real x[] = { 0. };
    Real u[] = { 0. };
    EXPECT_THROW_MSG(e.evaluate(1_D, 0., x, 1, u),
                     "Function evaluator failed: Expression is empty.");
}
