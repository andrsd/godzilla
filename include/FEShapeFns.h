...
DenseVector<DenseVector<Real, DIM>, N_ELEM_NODES>
...
template <>
DenseVector<DenseVector<Real, 1>, 2>
...
template <>
DenseVector<DenseVector<Real, 2>, 3>
...
/// Calculate shape function gradients per elements
///
/// @tparam ELEM_TYPE Element type
/// @tparam DIM Spatial dimension
/// @tparam N_ELEM_NODES Number of nodes per element
/// @param coords Mesh coordinates
/// @param connect Connectivity array
/// @param volumes Element volumes
/// @param grad_shfns Array to store computed gradients in
template <ElementType ELEM_TYPE, Int DIM, Int N_ELEM_NODES = get_num_element_nodes(ELEM_TYPE)>
void
calc_grad_shape(const Array1D<DenseVector<Real, DIM>> & coords,
                ...
                Array2D<DenseVector<Real, DIM>> & grad_shfns)
{
    _F_;
    assert(connect.get_size() == grad_shfns.get_num_rows());
    for (godzilla::Int ie = 0; ie < connect.get_size(); ie++) {
        auto idx = connect.get(ie);
        auto elem_coord = coords.get_values(idx);
        DenseVector<DenseVector<Real, DIM>, N_ELEM_NODES> grads =
            grad_shape<ELEM_TYPE, DIM>(elem_coord, volumes(ie));
        for (Int i = 0; i < N_ELEM_NODES; i++)
            grad_shfns(ie, i) = grads(i);
    }
}
