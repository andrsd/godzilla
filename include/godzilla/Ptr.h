...
public:
    ...
    explicit Ptr(T * p) : ctrl_(nullptr)
    {
        if (p)
            this->ctrl_ = new ControlBlock(p);
    }
...
public:
    /// Allocate an array of new objects and return a `Ptr` to it
    ///
    /// @tparam T C++ type we point to
