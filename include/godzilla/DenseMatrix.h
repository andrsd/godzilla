...
template <typename T>
using DynDenseMatrixSymm = DenseMatrixSymm<T, -1>;
...
public:
    ...
    DenseMatrix(const DynDenseMatrixSymm<T> & symm)
    {
        resize(symm.get_num_rows(), symm.get_num_cols());
    ...
    // DenseMatrix<T, ROWS, COLS>
    // operator*(const DynDenseMatrixSymm<T, COLS> & x) const
    // {
    //     return mult(x);
    // }
