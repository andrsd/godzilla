...
excluded_srcfiles += main.cpp
relative_excluded_srcfiles := $(foreach i, $(excluded_srcfiles), $(shell find $(SRC_DIRS) -name $(i)))
ifeq ($(LIBRARY_SUFFIX),yes)
excluded_objects	    := $(patsubst %.cpp, %_with$(app_LIB_SUFFIX).$(obj-suffix), $(relative_excluded_srcfiles))
else
excluded_objects	    := $(patsubst %.cpp, %.$(obj-suffix), $(relative_excluded_srcfiles))
endif

find_excludes     := $(foreach i, $(excluded_srcfiles), -not -name $(i))
srcfiles    := $(shell find $(SRC_DIRS) -regex "[^\#~]*\.cpp" $(find_excludes))
...
$(foreach srcsubdir,$(unity_srcsubdirs),$(eval $(call unity_file_rule,$(call unity_unique_name,$(unity_src_dir),$(APPLICATION_DIR),$(srcsubdir)),$(shell find $(srcsubdir) -maxdepth 1 \( -type f -o -type l \) -regex "[^\#~]*\.cpp"),$(srcsubdir),$(unity_src_dir))))
...
app_non_unity_srcfiles := $(shell find $(non_unity_srcsubdirs) -maxdepth 1 \( -type f -o -type l \) -regex "[^\#~]*\.cpp" $(find_excludes))
...
# object files
ifeq ($(LIBRARY_SUFFIX),yes)
objects	    := $(patsubst %.cpp, %_with$(app_LIB_SUFFIX).$(obj-suffix), $(srcfiles))
else
...
test_srcfiles    := $(shell find $(TEST_SRC_DIRS) -regex "[^\#~]*\.cpp" $(find_excludes) 2>/dev/null)
...
ifeq ($(LIBRARY_SUFFIX),yes)
  test_objects:= $(patsubst %.cpp, %_with$(app_LIB_SUFFIX).$(obj-suffix), $(test_srcfiles))
else
  test_objects:= $(patsubst %.cpp, %.$(obj-suffix), $(test_srcfiles))
endif
...
# plugin files
plugfiles   := $(shell find $(PLUGIN_DIR) -regex "[^\#~]*\.cpp" 2>/dev/null)
...
# plugins
plugins	    := $(patsubst %.cpp, %-$(METHOD).plugin, $(plugfiles))
...
app_analyzer := $(patsubst %.cpp, %.plist.$(obj-suffix), $(srcfiles))
