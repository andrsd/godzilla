...
#ifdef PETSC_HAVE_HYPRE
    if (type == EUCLID)
        PETSC_CHECK(PCHYPRESetType(this->obj_, "euclid"));
    else if (type == PILUT)
        PETSC_CHECK(PCHYPRESetType(this->obj_, "pilut"));
    else if (type == PARASAILS)
        PETSC_CHECK(PCHYPRESetType(this->obj_, "parasails"));
    else if (type == BOOMERAMG)
        PETSC_CHECK(PCHYPRESetType(this->obj_, "boomeramg"));
    else if (type == AMS)
        PETSC_CHECK(PCHYPRESetType(this->obj_, "ams"));
    else if (type == ADS)
        PETSC_CHECK(PCHYPRESetType(this->obj_, "ads"));
#else
    throw Exception("PETSc was not built with HYPRE.");
#endif
}
...
#ifdef PETSC_HAVE_HYPRE
    const char * name;
    PETSC_CHECK(PCHYPREGetType(this->obj_, &name));
...
PCHypre::ams_set_interior_nodes(const Vector & interior)
{
    ...
    PETSC_CHECK(PCHYPREAMSSetInteriorNodes(this->obj_, interior));
    #endif
#else
    throw Exception("PETSc was not built with HYPRE.");
#endif
...
PCHypre::set_alpha_poisson_matrix(const Matrix & A)
{
    CALL_STACK_MSG();
#ifdef PETSC_HAVE_HYPRE
    PETSC_CHECK(PCHYPRESetAlphaPoissonMatrix(this->obj_, A));
#else
    throw Exception("PETSc was not built with HYPRE.");
#endif
}
...
PCHypre::set_beta_poisson_matrix(const Matrix & A)
{
    CALL_STACK_MSG();
#ifdef PETSC_HAVE_HYPRE
    PETSC_CHECK(PCHYPRESetBetaPoissonMatrix(this->obj_, A));
#else
    throw Exception("PETSc was not built with HYPRE.");
#endif
}
...
PCHypre::set_discrete_curl(const Matrix & C)
{
    CALL_STACK_MSG();
#ifdef PETSC_HAVE_HYPRE
    PETSC_CHECK(PCHYPRESetDiscreteCurl(this->obj_, C));
#else
    throw Exception("PETSc was not built with HYPRE.");
#endif
}
...
PCHypre::set_discrete_gradient(const Matrix & G)
{
    CALL_STACK_MSG();
#ifdef PETSC_HAVE_HYPRE
    PETSC_CHECK(PCHYPRESetDiscreteGradient(this->obj_, G));
#else
    throw Exception("PETSc was not built with HYPRE.");
#endif
}
...
PCHypre::set_edge_constant_vectors(const Vector & oz, const Vector & zo)
{
    CALL_STACK_MSG();
#ifdef PETSC_HAVE_HYPRE
    PETSC_CHECK(PCHYPRESetEdgeConstantVectors(this->obj_, oz, zo, nullptr));
#else
    throw Exception("PETSc was not built with HYPRE.");
#endif
}
...
PCHypre::set_edge_constant_vectors(const Vector & ozz, const Vector & zoz, const Vector & zzo)
{
    CALL_STACK_MSG();
#ifdef PETSC_HAVE_HYPRE
    PETSC_CHECK(PCHYPRESetEdgeConstantVectors(this->obj_, ozz, zoz, zzo));
#else
    throw Exception("PETSc was not built with HYPRE.");
#endif
}
