                                       ...
                                       String region)
{
    ...
    if (region.length() == 0) {
        ...
        for (auto & val : ids) {
            add_weak_form_residual_block(WeakForm::F0, fid, f0, label, val, 0);
            add_weak_form_residual_block(WeakForm::F1, fid, f1, label, val, 0);
        }
    }
}
                                                ...
                                                String boundary)
{
    ...
    auto ids = label.get_values();
    for (auto & val : ids) {
        add_weak_form_residual_block(WeakForm::BND_F0, fid, f0, label, val, 0);
        add_weak_form_residual_block(WeakForm::BND_F1, fid, f1, label, val, 0);
    }
}

void
FEProblemInterface::add_jacobian_block(FieldID fid,
                                       FieldID gid,
                                       JacobianFunc * g0,
                                       JacobianFunc * g1,
                                       JacobianFunc * g2,
...
void
FEProblemInterface::add_jacobian_preconditioner_block(FieldID fid,
                                                      FieldID gid,
                                                      JacobianFunc * g0,
                                                      JacobianFunc * g1,
                                                      JacobianFunc * g2,
...
void
FEProblemInterface::add_boundary_jacobian_block(FieldID fid,
                                                FieldID gid,
                                                JacobianFunc * g0,
                                                JacobianFunc * g1,
                                                JacobianFunc * g2,
                                       ...
                                       Scalar elem_vec[])
{
    ...
    expect_true(q_n_comp == 1,
                fmt::format("Only supports scalar quadrature, not {} components", q_n_comp));

    Int dim_embed = cell_geom->dimEmbed;
    expect_true(cell_geom->dim == q_dim,
                fmt::format("FEGeom dim {} != {} quadrature dim", cell_geom->dim, q_dim));
