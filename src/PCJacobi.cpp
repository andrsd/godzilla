...
{
    CALL_STACK_MSG();
    PETSC_CHECK(PCJacobiSetType(this->obj_, static_cast<PCJacobiType>(type)));
}
...
PCJacobi::get_type() const
{
    CALL_STACK_MSG();
    PCJacobiType type;
    PETSC_CHECK(PCJacobiGetType(this->obj_, &type));
...
PCJacobi::get_fix_diagonal() const
{
    CALL_STACK_MSG();
    PetscBool flag;
    PETSC_CHECK(PCJacobiGetFixDiagonal(this->obj_, &flag));
...
PCJacobi::get_use_abs() const
{
    CALL_STACK_MSG();
    PetscBool flag;
    PETSC_CHECK(PCJacobiGetUseAbs(this->obj_, &flag));
    return flag == PETSC_TRUE;
}
...
{
    CALL_STACK_MSG();
    PETSC_CHECK(PCJacobiSetFixDiagonal(this->obj_, flag ? PETSC_TRUE : PETSC_FALSE));
}
...
{
    CALL_STACK_MSG();
    PETSC_CHECK(PCJacobiSetUseAbs(this->obj_, flag ? PETSC_TRUE : PETSC_FALSE));
}
