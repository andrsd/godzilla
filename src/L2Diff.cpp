...
L2Diff__invoke_delegate(Int /* dim */,
                        Real time,
                        const Real x[],
                        Int /* nc */,
                        Scalar u[],
...
L2Diff::compute()
{
    ...
    auto dpio = try_dynamic_ref_cast<DiscreteProblemInterface>(problem);
    GODZILLA_ASSERT_TRUE(dpio.has_value(), "Discrete problem is null");
