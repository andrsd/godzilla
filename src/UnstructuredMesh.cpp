...
void
UnstructuredMesh::create_face_set_labels(const std::map<Int, String> & names)
{
    CALL_STACK_MSG();
...
void
UnstructuredMesh::create_vertex_set_labels(const std::map<Int, String> & names)
{
    CALL_STACK_MSG();
...
UnstructuredMesh::get_num_cell_nodes(PolytopeType elem_type)
{
    ...
    case PolytopeType::HEXAHEDRON:
        return 8;
    default:
        throw InternalError(format("Unsupported type '{}'.", conv::to_str(elem_type)));
    }
}
...
UnstructuredMesh::is_my_cell(Int cell) const
{
    CALL_STACK_MSG();
    auto my_cells = get_label("my_cells");
    GODZILLA_ASSERT_TRUE(my_cells, "No my_cells label in the mesh");
